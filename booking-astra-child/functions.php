...
/* Full-width layout on front page - Tabligh style (uses Astra's page-builder template) */
...
/* Apply Islamic color scheme via Astra Customizer defaults */
...
function booking_astra_floating_whatsapp() {
	...
	<style>.booking-whatsapp-float{position:fixed;bottom:20px;right:20px;width:56px;height:56px;background:#25D366;color:#fff!important;border-radius:50%;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 12px rgba(0,0,0,.25);z-index:9999;transition:transform .2s}.booking-whatsapp-float:hover{transform:scale(1.1);color:#fff!important}</style>
	<?php
}

/* Add WhatsApp number to Customizer */
...
function booking_astra_customizer_defaults( $defaults ) {
	...
	$defaults['header-color-site-title']           = '#006B3F';  // Green
	$defaults['header-color-site-tagline']         = '#2d2d2d';
	$defaults['global-primary-button-bg-color']    = '#006B3F';  // Green
	$defaults['global-primary-button-text-color']  = '#FFFFFF';
	$defaults['global-primary-button-h-color']     = '#C9A227';  // Gold on hover
